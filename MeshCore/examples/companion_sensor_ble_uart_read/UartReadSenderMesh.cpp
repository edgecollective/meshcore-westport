...
void UartReadSenderMesh::pollUart(Stream& in) {
  while (in.available()) {
    int c = in.read();
    if (c < 0) {
      return;
...
void UartReadSenderMesh::onContactResponse(const ContactInfo& contact, const uint8_t* data, uint8_t len) {
...
bool UartReadSenderMesh::printSelfCard(Stream& out) {
...
bool UartReadSenderMesh::importTargetCard(const char* encoded) {
  if (!encoded) {
    return false;
  }
...
bool UartReadSenderMesh::setTargetPubKeyFromHex(const char* pub_key_hex) {
...
void UartReadSenderMesh::clearTarget() {
  memset(target_pub_key, 0, sizeof(target_pub_key));
  target_valid = false;
  awaiting_response = false;
  retry_pending = false;
...
bool UartReadSenderMesh::triggerManualSend() {
...
bool UartReadSenderMesh::storeTargetSelection() {
  if (!target_valid) {
    return false;
  }
...
bool UartReadSenderMesh::restoreStoredTargetSelection() {
...
bool UartReadSenderMesh::restoreTargetFromContacts() {
  ContactInfo contact;
...
bool UartReadSenderMesh::ensureTargetContact() {
  if (!target_valid) {
    return false;
  }
...
bool UartReadSenderMesh::sendSensorUpload(bool force_flood) {
...
unsigned long UartReadSenderMesh::calcResponseTimeout(bool direct, uint8_t path_len) const {
...
unsigned long UartReadSenderMesh::nextSendDelayMillis() const {
  return ((unsigned long)TELEMETRY_INTERVAL_SECS * 1000UL) + getRNG()->nextInt(0, 2000);
}

bool UartReadSenderMesh::hasFreshSample() const {
  if (!sample_valid) {
    return false;
  }
...
uint32_t UartReadSenderMesh::getSecondsUntilNextSend() const {
  if (!target_valid) {
    return 0;
  }
...
uint32_t UartReadSenderMesh::getSecondsSinceLastSample() const {
  if (!sample_valid) {
    return 0;
  }
