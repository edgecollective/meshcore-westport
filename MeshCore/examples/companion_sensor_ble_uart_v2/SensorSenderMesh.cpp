...
SensorSenderMesh::SensorSenderMesh(mesh::Radio& radio, mesh::RNG& rng, mesh::RTCClock& rtc, SimpleMeshTables& tables, DataStore& store)
  : MyMesh(radio, rng, rtc, tables, store), next_send_at(0), response_deadline(0), last_uart_sample_at(0),
    last_request_tag(0), last_battery_mv(0), last_temperature_x10(0),
    ...
    if (c == '\n') {
      if (!uart_line_discard && uart_line_len > 0) {
        ...
        if (parseUartLine(uart_line, parsed_node_id, parsed_temp_x10, parsed_battery_mv)) {
          ...
          Serial.printf("UART OK: n=%u t=%.1f b=%.3f\n",
                        (unsigned)node_id,
...
bool SensorSenderMesh::parseUartLine(const char* line, uint16_t& parsed_node_id, int16_t& parsed_temp_x10, uint16_t& parsed_battery_mv) {
  if (!line || strncmp(line, "MC,N=", 5) != 0) {
    StrHelper::strncpy(last_status, "uart bad: no MC,N=", sizeof(last_status));
    return false;
  }
  ...
  if (!end || end == line + 5 || node > 65535 || strncmp(end, ",T=", 3) != 0) {
    StrHelper::strncpy(last_status, "uart bad: node/T", sizeof(last_status));
    return false;
  }
  ...
  if (!temp_end || temp_end == end + 3 || strncmp(temp_end, ",B=", 3) != 0) {
    StrHelper::strncpy(last_status, "uart bad: temp/B", sizeof(last_status));
    return false;
  }
  ...
  if (!batt_end || batt_end == temp_end + 3 || *batt_end != 0) {
    StrHelper::strncpy(last_status, "uart bad: batt", sizeof(last_status));
    return false;
  }

  if (temp_c < -100.0f || temp_c > 150.0f) {
    StrHelper::strncpy(last_status, "uart bad: temp range", sizeof(last_status));
    return false;
  }

  if (batt_v < 0.0f || batt_v > 6.0f) {
    StrHelper::strncpy(last_status, "uart bad: batt range", sizeof(last_status));
    return false;
  }
...
bool SensorSenderMesh::hasFreshSample() const {
  if (!sample_valid) {
    return false;
  }
...
uint32_t SensorSenderMesh::getSecondsSinceLastSample() const {
  if (!sample_valid) {
    return 0;
  }
