...
bool MyMesh::upsertManualContact(const uint8_t* pub_key, const char* name, uint8_t type) {
  ...
  if (contact) {
    ...
    if (!mesh::Packet::isValidPathLen(contact->out_path_len)) {
      contact->out_path_len = OUT_PATH_UNKNOWN;
    }
  } else {
    ContactInfo temp;
    memset(&temp, 0, sizeof(temp));
    memcpy(temp.id.pub_key, pub_key, PUB_KEY_SIZE);
    StrHelper::strncpy(temp.name, name, sizeof(temp.name));
    temp.type = type;
    temp.out_path_len = OUT_PATH_UNKNOWN;
    temp.lastmod = now;
    temp.sync_since = 0;
    temp.shared_secret_valid = false;
...
void MyMesh::persistContactsNow() {
  saveContacts();
  dirty_contacts_expiry = 0;
}
